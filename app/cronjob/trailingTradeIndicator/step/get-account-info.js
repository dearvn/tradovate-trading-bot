...
const execute = async (logger, rawData) => {
  ...
  // Tradovate API weight for account info should be 10 for each request.
  // However, sometimes Tradovate changes API weight to 20 for some reason.
  ...
  const oldAccountInfo =
    JSON.parse(await cache.hget('trailing_trade_common', 'account-info')) || {};
  ...
  // Determine to reset Tradovate websocket

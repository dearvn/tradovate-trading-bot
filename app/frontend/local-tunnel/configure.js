...
const connect = async logger => {
  if (config.get('localTunnel.enabled') !== true) {
    ...
    await cache.hdel('trailing_trade_common', 'local-tunnel-url');
    return false;
  }
  ...
  try {
    ...
    reconnect(
      logger,
      `Local tunnel got an exception, try to connect after ${retryMs / 1000
      } secs.`,
      10000
    );
    return null;
  ...
  if (cachedLocalTunnelURL !== tunnel.url) {
    ...
    await cache.hset('trailing_trade_common', 'local-tunnel-url', tunnel.url);
  ...
  if (tunnel.url.includes(config.get('localTunnel.subdomain')) === true) {
    ...
    reconnect(
      logger,
      `Local tunnel is different with configured subdomain. Try after ${retryMs / 1000
      } secs.`,
      retryMs
    );
  }

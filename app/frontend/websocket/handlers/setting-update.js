...
const deleteAllCachedSymbolInfo = async logger => {
  await cache.hdel('trailing_trade_common', 'exchange-symbols');
  await cache.hdel('trailing_trade_common', 'exchange-info');

  const symbolKeys = await cache.hgetall(
    'trailing_trade_symbols:',
    'trailing_trade_symbols:*-symbol-info'
  );

  await Promise.all(
    Object.keys(symbolKeys).map(async key => {
      await cache.hdel('trailing_trade_symbols', key);
      //logger.info(`Removed trailing_trade_symbols:${key} cache`);
    })
  );
};
